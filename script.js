...
const XLSX_COLS=[['BASE',12],['PACOTE',16],['ROTA',14],['PRODUTO',46],['MOTIVO',22],['MOTORISTA',28],['DIAS PARADO',13],['VALOR (R$)',14],['JUSTIFICATIVA',34],['FOTO / ANEXAR FOTO',22],['RESOLVIDO',12],['',4],['TUTORIAL PARA ANEXAR FOTO',58]];
...
+'<font><b/><sz val="11"/><color rgb="FFCC7777"/><name val="Calibri"/></font>'
+'</fonts>'
+'<fills count="11">'
  ...
  grupos.forEach(g=>{
    ...
    linhas.forEach(({base,e})=>{
      ...
      rows+='<row r="'+r+'">'
        ...
        +cTxt(11,r,'',4)
        +cTxt(12,r,'Clique em ANEXAR FOTO > Inserir (ao lado de Início) > Imagens > Inserir do dispositivo > ícone paisagem com quadradinho.',15)
        +'</row>';
    });
    r++;
