...
function renderMotoristas(){
  ...
  const selected=ids.includes(motoristaSelecionado)?motoristaSelecionado:''; const filtered=selected?rows.filter(x=>String(x.e.driverId)===selected):[]; const historicoFiltered=selected?historicoRows.filter(x=>String(x.e.driverId)===selected):[]; const total=filtered.reduce((s,x)=>s+x.e.valor,0); const diasOfensor=new Set(), diasPorFaixa={ATE2:new Set(),'3A6':new Set(),'7A11':new Set()}; historicoFiltered.forEach(x=>{const d=Number(diasParado(x.e,String(x.day)+'T00:00:00')); diasOfensor.add(x.day); if(d>=1&&d<=2) diasPorFaixa.ATE2.add(x.day); else if(d>=3&&d<=6) diasPorFaixa['3A6'].add(x.day); else if(d>=7&&d<=11) diasPorFaixa['7A11'].add(x.day);}); const pacotes=new Set(filtered.map(x=>String(x.e.pacote||'')).filter(Boolean));
