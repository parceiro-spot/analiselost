...
function abrirModalOcultarDias(entries, contexto){
  ...
  modal.innerHTML='<div class="hide-days-card" role="dialog" aria-modal="true" aria-labelledby="hideDaysTitle">'
    +'<div class="hide-days-head"><div><h3 id="hideDaysTitle">OCULTAR DIAS POR ABA</h3><p>Marque as faixas que não devem aparecer em cada aba regional.</p></div><button type="button" class="overlay-close" data-hide-days-close>Fechar</button></div>'
    +'<div class="hide-days-group" data-hide-group="1"><h4>PRIMEIRA ABA — NÃO CONTER:</h4><div class="hide-days-options">'+options()+'</div></div>'
    +'<button type="button" class="btn-reset hide-days-add" data-add-hide-group>ADICIONAR MAIS</button>'
    +'<div class="hide-days-help">Você pode marcar 1–2 dias e 3–6 dias ao mesmo tempo. Cada aba tem uma seleção independente.</div>'
  ...
  modal.addEventListener('click',ev=>{
    ...
    if(ev.target.closest('[data-add-hide-group]')){
      ...
      group.innerHTML='<h4>'+(['PRIMEIRA','SEGUNDA','TERCEIRA','QUARTA'][count-1]||count+'ª')+' ABA — NÃO CONTER:</h4><div class="hide-days-options">'+options()+'</div><button type="button" class="btn-reset hide-days-remove" data-remove-hide-group>Remover esta aba</button>';
    ...
    if(ev.target.closest('[data-hide-days-confirm]')){
      const ocultos=new Set(); modal.querySelectorAll('.hide-days-group input:checked').forEach(input=>ocultos.add(input.value));
      const filtrados=(entries||[]).filter(e=>!ocultos.has(bucketOf(diasParado(e))));
      close(); baixarRetorno(filtrados,contexto+(ocultos.size?' · faixas não contidas: '+Array.from(ocultos).map(bucket=>BUCKET_LABELS[bucket]).join(', '):''));
    }
  });
}
