...
const XLSX_COLS=[['BASE',12],['PACOTE',16],['ROTA',14],['PRODUTO',46],['MOTIVO',22],['MOTORISTA',28],['DIAS PARADO',13],['VALOR (R$)',14],['JUSTIFICATIVA',34],['FOTO / ANEXAR FOTO',22],['RESOLVIDO',12]];
...
function buildSheetXml(grupos, contexto){
  ...
  merges.push('A'+r+':K'+r);
  r++; rows+='<row r="'+r+'" ht="30" customHeight="1">'+cTxt(0,r,'Tutorial: clique em ANEXAR FOTO > depois clique em Inserir, na parte de cima ao lado de Início > Imagens > Inserir do dispositivo > ícone de paisagem com quadradinho.',4);
  for(let c=1;c<11;c++) rows+='<c r="'+colName(c)+r+'" s="4"/>';
  rows+='</row>'; merges.push('A'+r+':K'+r);
  grupos.forEach(g=>{
    ...
    for(let c=1;c<11;c++) rows+='<c r="'+colName(c)+r+'" s="2"/>';
    rows+='</row>';
    merges.push('A'+r+':K'+r);
    ...
    rows+='<row r="'+r+'">'+cTxt(0,r,'Total '+(REGIONAL_LABELS[g.regional]||g.regional),10)
      ...
      +cNum(7,r,valorReg,11)+cTxt(8,r,'',4)+cTxt(9,r,'',4)+cTxt(10,r,'',4)+'</row>';
    merges.push('A'+r+':G'+r);
  });
  linhaVazia();
